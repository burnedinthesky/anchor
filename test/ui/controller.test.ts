...
import { makeMarker, makeRecord, makeEvent, cardHost, cardRoot, flush, rect } from "./helpers";
...
function stubCardSize(): void {
  const host = cardHost();
  const inner = host?.shadowRoot?.querySelector(".card") as HTMLElement | null;
...
describe("CitationController wiring", () => {
  ...
  it("queues events before full-text fetch resolves, then flushes", async () => {
    ...
    expect(h.div.querySelectorAll("button.anchor-cite-btn").length).toBe(0);

    h.resolvePages();
    await startP;
    expect(h.div.querySelectorAll("button.anchor-cite-btn").length).toBe(1);
  });
  ...
  it("null resolution -> empty state with Scholar search on rawText", async () => {
    ...
    firstButton(h.div).click();
    await flush();
    const link = cardRoot().querySelector(".actions a") as HTMLAnchorElement;
  ...
  it("completeness empty -> empty state", async () => {
    const h = harness({
      lookup: async () =>
        makeRecord({ completeness: "empty", scholarUrl: "https://scholar.google.com/scholar?q=x" }),
    });
    const startP = h.controller.start();
    h.emit(makeEvent(1, h.div));
    h.resolvePages();
  ...
  it("MetadataLookupError -> error, Retry re-fetches and transitions to success", async () => {
    const lookup = vi
      .fn()
      .mockRejectedValueOnce(new MetadataLookupError("boom", { status: 500 }))
    ...
    firstButton(h.div).click();
    await flush();
    const retry = cardRoot().querySelector(".btn.primary") as HTMLButtonElement;
  ...
  it("single-card invariant: opening a second card closes the first", async () => {
    ...
    const btns = h.div.querySelectorAll<HTMLButtonElement>("button.anchor-cite-btn");
    btns[0]!.click();
    btns[1]!.click();
    await flush();
    expect(document.querySelectorAll("[data-anchor-card-host]").length).toBe(1);
  });
