...
describe("PreviewCard states", () => {
  ...
  it("success renders title link, authors +N more, metrics, related, versions, actions", () => {
    ...
    expect(root.querySelector(".authors")!.textContent).toContain("+2 more");
    expect(root.querySelector(".meta")!.textContent).toBe("2021 · NeurIPS");
    expect(root.querySelector(".metrics")!.textContent).toBe("Cited by 142");
  ...
  it("author truncation omitted when few authors", () => {
    ...
    expect(cardRoot().querySelector(".authors")!.textContent).toBe("Solo Author");
  });
  ...
  it("partial hides empty sections and shows the note", () => {
    ...
    expect(root.querySelector(".note")!.textContent).toContain("unavailable");
  ...
  it("empty state links Search on Google Scholar to the scholar url", () => {
    ...
    card.showEmpty(url);
    const root = cardRoot();
    expect(root.querySelector(".empty")!.textContent).toContain("Couldn't resolve");
  ...
  it("error state Retry invokes the callback", () => {
    ...
    const root = cardRoot();
    expect(root.querySelector(".error")!.textContent).toContain("Couldn't load");
