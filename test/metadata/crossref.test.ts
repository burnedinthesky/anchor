...
describe("Crossref client", () => {
  it("extracts DOI, strips JATS abstract, maps authors/year/venue", async () => {
    ...
    const r = await client.bibliographic("Graph Learning at Scale, Lovelace 2020", {
      title: "Graph Learning at Scale",
      year: 2020,
    });
  ...
  it("sends query.bibliographic, rows=1, select and mailto", async () => {
    const fetchMock = routeFetch([
      { match: "api.crossref.org", respond: jsonResponse(crossrefBody({ DOI: "10.1/x", title: ["Graph Learning"] })) },
    ]);
    const client = createCrossrefClient(makeHttp(fetchMock), MAILTO);
    await client.bibliographic("Graph Learning", { title: "Graph Learning" });
        ...
        routeFetch([
          {
            ...
            respond: jsonResponse(
              crossrefBody({
                DOI: "10.9/wrong",
                title: ["Completely Unrelated Marine Biology Survey"],
