...
describe("OpenAlex client", () => {
  it("maps every field from a DOI lookup and reconstructs the abstract", async () => {
    ...
    expect(r!.abstract).toBe("Deep learning models the citation of the graph");
  ...
  it("resolves related works with exactly ONE batched request containing all ids", async () => {
    ...
    const batchCalls = calledUrls(fetchMock).filter((u) => u.includes("filter=openalex_id"));
    expect(batchCalls).toHaveLength(1);
    expect(decodeURIComponent(batchCalls[0]!)).toContain("W2001|W2002");

    expect(r!.related!.map((x) => x.title)).toEqual(["Related One", "Related Two"]);
    expect(r!.related![0]!.scholarUrl).toContain("scholar.google.com/scholar?q=");
  ...
  it("caps related at MAX_RELATED, dropping the paper itself and duplicate titles", async () => {
    const work = openAlexWork({
      ...
      related_works: Array.from({ length: 8 }, (_, i) => `https://openalex.org/W${i}`),
    });
    const batch = {
      results: [
        { id: "https://openalex.org/WSELF", display_name: "Self Echo", publication_year: 2021 },
        { id: "https://openalex.org/W1", display_name: "Dup Title", publication_year: 2019 },
        { id: "https://openalex.org/W2", display_name: "Dup Title", publication_year: 2019 },
        { id: "https://openalex.org/W3", display_name: "Unique A", publication_year: 2018 },
        { id: "https://openalex.org/W4", display_name: "Unique B", publication_year: 2017 },
      ],
    };
    ...
    const batchUrl = calledUrls(fetchMock).find((u) => u.includes("filter=openalex_id"))!;
  ...
  it("returns null on 404 (no match)", async () => {
    const client = createOpenAlexClient(
      makeHttp(routeFetch([{ match: "/works/https://doi.org/", respond: statusResponse(404) }])),
      MAILTO
    );
