...
import { routeFetch, jsonResponse, statusResponse, makeHttp, calledUrls } from "./helpers";
...
describe("Semantic Scholar client", () => {
  it("fetches a paper by DOI and maps abstract + recommendations", async () => {
    ...
    const recUrl = calledUrls(fetchMock).find((u) => u.includes("/recommendations/"))!;
  ...
  it("falls back to title search when the DOI lookup 404s", async () => {
    const fetchMock = routeFetch([
      { match: "/graph/v1/paper/DOI:", respond: statusResponse(404) },
      {
        ...
        respond: jsonResponse({ data: [{ paperId: "p2", title: "Found By Title", abstract: "abs" }] }),
      },
    ]);
    const client = createSemanticScholarClient(makeHttp(fetchMock));
    const r = await client.enrich({ doi: "10.1/missing", title: "Found By Title" }, { needRelated: false });
  ...
  it("does not require mailto and skips recommendations when not needed", async () => {
    const fetchMock = routeFetch([
      { match: "/graph/v1/paper/search", respond: jsonResponse({ data: [{ title: "T", abstract: "a" }] }) },
    ]);
    const client = createSemanticScholarClient(makeHttp(fetchMock));
    await client.enrich({ title: "T" }, { needRelated: false });
    expect(calledUrls(fetchMock).some((u) => u.includes("/recommendations/"))).toBe(false);
  });
});
