...
import { rectsForSpan, rectForItemSpan } from "../../src/citations/detect/geometry";
...
describe("geometry mapping (§4.3)", () => {
  it("maps a single-item [1] rect with correct y-flip and size", () => {
    ...
    const items: TextItem[] = [
      { str: "[1]", transform: [10, 0, 0, 10, 100, 730], width: 15, height: 10, fontName: "b", dir: "ltr" },
    ];
  ...
  it("emits one rect per item for a span crossing multiple items", () => {
    const items: TextItem[] = [
      { str: "[3]", transform: [10, 0, 0, 10, 100, 700], width: 15, height: 10, fontName: "b", dir: "ltr" },
      { str: "–", transform: [10, 0, 0, 10, 120, 700], width: 5, height: 10, fontName: "b", dir: "ltr" },
      { str: "[5]", transform: [10, 0, 0, 10, 130, 700], width: 15, height: 10, fontName: "b", dir: "ltr" },
    ];
    const { text, map } = concatPage(items);
    const m = findNumericMatches(text)[0]!;
  ...
  it("computes a fractional sub-span rect within one item", () => {
    ...
    const items: TextItem[] = [
      { str: "AB[7]", transform: [10, 0, 0, 10, 0, 700], width: 50, height: 10, fontName: "b", dir: "ltr" },
    ];
    const { text, map } = concatPage(items);
    const m = findNumericMatches(text)[0]!;
    const rects = rectsForSpan(m.start, m.end, map, items, vp);
