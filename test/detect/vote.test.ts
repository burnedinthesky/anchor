...
import { voteScheme, DocumentCitationDetector } from "../../src/citations/detect/index";
...
describe("dominant-scheme vote (fixtures)", () => {
  it("IEEE numeric paper → numeric", () => {
    expect(new DocumentCitationDetector(fixtures.numeric).dominantScheme).toBe("numeric");
  });
  it("ACM author-year paper → author-year", () => {
    expect(new DocumentCitationDetector(fixtures.authorYear).dominantScheme).toBe(
      "author-year"
    );
  });
  it("superscript paper → superscript", () => {
    expect(new DocumentCitationDetector(fixtures.superscript).dominantScheme).toBe(
      "superscript"
    );
  });
  it("references-section bracket labels do NOT distort the vote", () => {
    ...
    expect(new DocumentCitationDetector(fixtures.mixed).dominantScheme).toBe(
      "author-year"
    );
  });
});
