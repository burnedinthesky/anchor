...
async function loadPages(
    path: string
): Promise<{ pages: PDFTextContent[]; viewports: any[] }> {
    const doc = await getDocument({ data: new Uint8Array(readFileSync(path)) })
        .promise;
...
function checkRects(markers: CitationMarker[], viewport: any, label: string) {
    for (const m of markers) {
        assert(
            m.rect.w > 0 && m.rect.h > 0,
            `${label}: degenerate rect on ${m.rawText}`
        );
        assert(
            m.rect.x >= -1 &&
                m.rect.y >= -1 &&
                m.rect.x + m.rect.w <= viewport.width + 1 &&
                m.rect.y + m.rect.h <= viewport.height + 1,
...
// --- Numeric fixture -------------------------------------------------------
{
    const { pages, viewports } = await loadPages(
        "test/fixtures/pdf/numeric.pdf"
    );
    const det = new DocumentCitationDetector(pages);
    assert(
        det.dominantScheme === "numeric",
        `numeric.pdf voted ${det.dominantScheme}`
    );
    const all = pages.flatMap((p, i) => det.detect(i + 1, p, viewports[i]));
    console.log(
        `[numeric] scheme=${det.dominantScheme} markers=${all.length}: ${[...new Set(all.map((m) => m.rawText))].join(" ")}`
    );
    ...
    assert(
        ok.length === resolved.length,
        `numeric.pdf: ${resolved.length - ok.length} markers failed to resolve`
    );
    for (const { m, r } of ok) {
        console.log(
            `  ${m.rawText} -> ordinal ${m.ordinal}: "${r!.rawText.slice(0, 70)}..." doi=${r!.doi ?? "-"}`
        );
        assert(
            r!.rawText.length > 20,
            `numeric.pdf: suspiciously short entry for ${m.rawText}`
        );
    }
}
...
{
    const { pages, viewports } = await loadPages(
        "test/fixtures/pdf/author-year.pdf"
    );
    const det = new DocumentCitationDetector(pages);
    assert(
        det.dominantScheme === "author-year",
        `author-year.pdf voted ${det.dominantScheme}`
    );
    const all = pages.flatMap((p, i) => det.detect(i + 1, p, viewports[i]));
    console.log(
        `[author-year] scheme=${det.dominantScheme} markers=${all.length}: ${[...new Set(all.map((m) => m.rawText))].join(" | ")}`
    );
    ...
    assert(
        ok.length >= Math.ceil(all.length / 2),
        "author-year.pdf: fewer than half of markers resolved"
    );
}
...
{
    const { pages, viewports } = await loadPages(
        "test/fixtures/pdf/superscript.pdf"
    );
    const det = new DocumentCitationDetector(pages);
    assert(
        det.dominantScheme === "superscript",
        `superscript.pdf voted ${det.dominantScheme}`
    );
    const all = pages.flatMap((p, i) => det.detect(i + 1, p, viewports[i]));
    console.log(
        `[superscript] scheme=${det.dominantScheme} markers=${all.length}: ${[...new Set(all.map((m) => m.rawText))].join(" ")}`
    );
