    ...
    for (const pageLines of doc) {
        ...
        for (const line of pageLines) {
            ...
            for (const span of line) {
                ...
                x +=
                    font.widthOfTextAtSize(span.text, size) +
...
const t = (text: string, size?: number, rise?: number): Span => ({
    text,
    size,
    rise,
});
...
const numeric: Doc = [
    ...
    [
        ...
        [
            t(
                "[2] C. Doe. Wide models. In Proc. NeurIPS, 2020. doi:10.1109/5.771073."
            ),
        ],
...
const superscript: Doc = [
    [
        [
            t("Recent studies "),
            t("1", 7, 4),
            t(" have shown significant gains."),
        ],
        [
            t("Other work "),
            t("2,3", 7, 4),
            t(" reports similar results in 42 trials."),
        ],
        [
            t("Follow-up analysis "),
            t("4", 7, 4),
            t(" extends this line of work."),
        ],
    ],
    [
        [t("References")],
...
async function main(): Promise<void> {
    ...
    for (const [name, doc] of docs) {
        ...
        console.log(
            `wrote test/fixtures/pdf/${name}.pdf (${bytes.length} bytes)`
        );
    }
}
