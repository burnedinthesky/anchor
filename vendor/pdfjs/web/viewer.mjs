...
class Autolinker {
  ...
  static findLinks(text) {
    // ANCHOR PATCH: The upstream literal uses an ES2024 unicodeSets (`v` flag)
    // regex with set subtraction, which web-ext's bundled (older) JS parser
    // rejects as a syntax error, blocking `web-ext lint`. Autolinking clickable
    // URLs in the text layer is a non-essential decoration, so it is replaced
    // with a standard-flag URL/email matcher. Group 1 still captures the email
    // domain, as the loop below relies on `match[1]`.
    this.#regex ??= /\b(?:https?:\/\/|mailto:|www\.)[^\s<>]+|[^\s<>@]+@([^\s<>@]+(?:\.[^\s<>@]+)+)/gm;
...
PDFPrintServiceFactory.initGlobals(PDFViewerApplication);
{
  ...
  var validateFileURL = function (file) {
    // ANCHOR PATCH: This extension is a dedicated PDF reader that intentionally
    // loads arbitrary cross-origin http(s) PDFs (granted via <all_urls> host
    // permission). The upstream same-origin check would reject every real PDF,
    // so it is disabled here. Only http(s) targets are accepted; other schemes
    // (file:, data:, javascript:, etc.) are still rejected as a safety guard.
    if (!file) {
      return;
    }
    const fileOrigin = URL.parse(file, window.location);
    if (fileOrigin && (fileOrigin.protocol === "http:" || fileOrigin.protocol === "https:")) {
      return;
    }
