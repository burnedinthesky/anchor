...
export class DocumentCitationDetector implements CitationDetector {
    ...
    constructor(pages: PDFTextContent[]) {
        ...
        if (rb) {
            ...
            this.refBoundary = {
                ...
                charIndex: pt
                    ? (pt.itemStarts[rb.firstItem] ?? pt.text.length)
                    : 0,
            };
        } else {
        ...
        for (let p = 0; p < pages.length; p++) {
            ...
            if (limit <= 0) continue;
            numeric += findNumericMatches(text).filter(
                (m) => m.start < limit
            ).length;
            authorYear += findAuthorYearMatches(text).filter(
                (m) => m.start < limit
            ).length;
    ...
    private charLimit(pageNumber: number, textLen: number): number {
        ...
        if (pageNumber === this.refBoundary.pageNumber)
            return this.refBoundary.charIndex;
        return 0;
    }
