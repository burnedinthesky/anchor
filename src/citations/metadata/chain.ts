 ...
 */
import { CACHE_TTL_DAYS, DEFAULT_MAILTO, buildScholarUrl } from "../types";
import type {
    MetadataProvider,
    MetadataSource,
    PaperRecord,
...
import { MetadataCache, createCacheStore, type CacheStore } from "./cache";
...
const SOURCE_ORDER: MetadataSource[] = [
    "openalex",
    "crossref",
    "semanticscholar",
];
    ...
    private async doLookup(ref: ResolvedReference): Promise<PaperRecord> {
        ...
        if (doi) {
            ...
            const r = await this.attempt(() =>
                this.openalex.byDoi(doi as string)
            );
            hadError ||= r.errored;
            oa = r.value;
            // §10: a clean 404 (not an error) falls through to title search.
            if (!oa && !r.errored) {
                const t = await this.attempt(() =>
                    this.openalex.byTitle(titleQuery)
                );
                hadError ||= t.errored;
                oa = t.value;
            }
